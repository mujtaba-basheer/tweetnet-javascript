...
const handleResponse = (task, limit_exceeded) => {
    if (!limit_exceeded)
        return;
    try {
...
window.addEventListener("load", async () => {
    try {
        ...
        // MY TWEETS
        {
            const { author_details, tweets, limit_exceeded } = await apiCall.getReq("/user/my-tweets");
            if (limit_exceeded)
                handleResponse("your", true);
                    ...
                    formDiv.classList.add("w-form");
                    {
                        ...
                        formEl.classList.add("flex-vertical");
                        {
                            ...
                            formEl.addEventListener("submit", async function (ev) {
                                ...
                                // LIKES
                                try {
                                    const likes = await apiCall.getReq(`/user/like/${tid}`);
                                ...
                                Object.keys(msgMap).forEach((x) => formEl.querySelector(`.${msgMap[x]}`).classList.add("hide"));
                                successEl.classList.add("hide");
                                ...
                                // }
                                try {
                                    const resp = await apiCall.postReq("/user/forward-tweets", {
                                        id: tid,
                                        tasks,
                                    });
        ...
        // MY TODO
        {
                                ...
                                inputEl.style.zIndex = "-1";
                                {
                                    inputEl.addEventListener("change", async () => {
                                        for (const tweet of tweetsToLike) {
                                            if (inputEl.checked) {
                                                try {
                                                    const completeEl = document.getElementById(`like-complete`);
                                                    completeEl.style.display = "block";
                                                    const swiperEl = document.getElementById(`like-overall`);
                                                    swiperEl.style.display = "none";
                    ...
                    document
                        ...
                        try {
                            ...
                            containerElLike
                                ...
                                .forEach((x) => x.remove());
                            for (let i = 0; i < tweetsToLike.length; i++) {
                                const tweet = tweetsToLike[randomArray[i]];
                                const { id: tid, text, created_at, author_details } = tweet;
                                // creating tweet element
                                const rootEl = document.createElement("div");
                                rootEl.classList.add("swiper-slide");
            ...
            {
                try {
                    let { tweets: tweetsToReply, limit_exceeded: limit_exceeded_reply } = await apiCall.getReq("/user/tweets/reply");
                    const replyLoader = $(document.getElementById("comment-preloader"));
                    const containerElReply = document.getElementById("comment-swiper");
                    if (limit_exceeded_reply)
                                ...
                                formEl.classList.add("form-twitter");
                                {
                                    ...
                                    boxDiv.classList.add("form-card");
                                    {
                                        ...
                                        checkboxesContainer.classList.add("checkboxes-wrapper");
                                        {
                                            const replyContainer = document.createElement("div");
                                            replyContainer.classList.add("column");
                                            replyContainer.setAttribute("data-w-id", "48652e03-98b1-c9e0-2cfc-1cf2558c7403");
                                            {
                                            ...
                                            replyInputContainer.classList.add("comment-wrapper");
                                            {
                                                ...
                                                xComment.setAttribute("data-w-id", "7d49dde2-22af-3af3-4d2a-6779b7115ead");
                                                {
                                                    const imgEl = document.createElement("img");
                                                    imgEl.classList.add("cover-image");
                                                    imgEl.src =
                    ...
                    document
                        ...
                        try {
                            ...
                            containerElReply
                                ...
                                .forEach((x) => x.remove());
                            for (let i = 0; i < tweetsToReply.length; i++) {
                                const tweet = tweetsToReply[randomArray[i]];
                                const { id: tid, text, created_at, author_details } = tweet;
                                // creating tweet element
                                const rootEl = document.createElement("div");
                                rootEl.classList.add("swiper-slide");
                    ...
                    document
                        ...
                        try {
                            ...
                            containerElRetweet
                                ...
                                .forEach((x) => x.remove());
                            for (let i = 0; i < tweetsToRetweet.length; i++) {
                                const tweet = tweetsToRetweet[randomArray[i]];
                                const { id: tid, text, created_at, author_details } = tweet;
                                // creating tweet element
                                const rootEl = document.createElement("div");
                                rootEl.classList.add("swiper-slide");
